...
{
	Quack head;
	head = (node *)malloc(sizeof(struct node));     // allocate memory for head node
	if ( head == NULL ){					 // exception no memory
		fpirntf(stderr, "No memory\n");
		exit(EXIT_FAILURE);
	}else{
...
void push(int data, Quack head)
{
	...
	newnode = (node *)malloc(sizeof(struct node));  // allocate memory for this new node
	if ( newnode == NULL ){
